...
public class SCR_Pool : MonoBehaviour {
	...
	private static GameObject GetFreeObjectFromEntry (PoolEntry entry, GameObject prefab) {
		...
		if(nullObjectCount >= entry.list.Count)
		{
			entry.list.Clear();
        }
		if (result == null) {
    ...
    private static GameObject GetFreeObjectFromEntry(PoolEntry entry, GameObject prefab, Transform parent)
    {
        GameObject result = null;
        foreach (GameObject gameObject in entry.list)
        {
    ...
    public static GameObject GetFreeObject(GameObject prefab, Transform parent)
    {
        PoolEntry result = null;
        foreach (PoolEntry entry in entries)
        {
        ...
        return GetFreeObjectFromEntry(result, prefab, parent);
    }
