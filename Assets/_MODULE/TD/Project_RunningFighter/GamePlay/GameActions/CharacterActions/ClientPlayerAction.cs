...
{
    public sealed class ClientPlayerAction
    {
        private List<GameAction> m_PlayingActions = new List<GameAction>();
        ...
        public ClientPlayerAction(ClientCharacter clientCharacter)
        {
        ...
        private int FindAction(GameActionID actionID, bool anticipatedOnly)
        {
        ...
        {
            if (!ClientCharacter.IsAnimating() && GameAction.ShouldClientAnticipate(ClientCharacter, ref data))
            {
                var actionFX = GameActionFactory.CreateActionFromData(ref data);
        ...
        public void PlayAction(ref ActionRequestData data)
        {
            ...
            var actionFX = anticipatedActionIndex >= 0 ?
                           m_PlayingActions[anticipatedActionIndex] :
                           GameActionFactory.CreateActionFromData(ref data);
            ...
            else if (anticipatedActionIndex >= 0)
            {
                ...
                GameActionFactory.ReturnAction(removedAction);
            }
        }
        ...
        public void CancelAllActionsWithSamePrototypeID(GameActionID actionID)
        {
            for (int i = m_PlayingActions.Count - 1; i >= 0; --i)
            {
