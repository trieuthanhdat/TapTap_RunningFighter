...
namespace Project_RunningFighter.Infrastruture
{
    public class UpdateRunner : MonoBehaviour
    {
        ...
        public void Subscribe(Action<float> onUpdate, float updatePeriod)
        {
            ...
            {
                Debug.LogError("UPDATE RUNNER:Can't subscribe to a local function that can go out of scope and can't be unsubscribed from");
                return;
            }

            if (onUpdate.Method.ToString().Contains("<")) // Detect
            {
                Debug.LogError("UPDATE RUNNER:Can't subscribe with an anonymous function that cannot be Unsubscribed, by checking for a character that can't exist in a declared method name.");
                return;
            }
