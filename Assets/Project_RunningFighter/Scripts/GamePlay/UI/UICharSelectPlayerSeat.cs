...
namespace Project_RunningFighter.Gameplay.UI
{
    ...
    public class UICharSelectPlayerSeat : MonoBehaviour
    {
        ...
        private NetworkCharacterSelection.SeatState m_State;
        ...
        public void Initialize(int seatIndex)
        {
            ...
            m_State = NetworkCharacterSelection.SeatState.Inactive;
            m_PlayerNumber = -1;
            ConfigureStateGraphics();
        }

        public void SetState(NetworkCharacterSelection.SeatState state, int playerIndex, string playerName)
        {
            ...
            if (m_State == NetworkCharacterSelection.SeatState.Inactive)
                m_PlayerNumber = -1;
            ConfigureStateGraphics();
        }
        ...
        {
            return m_State == NetworkCharacterSelection.SeatState.LockedIn;
        }
        ...
        private void ConfigureStateGraphics()
        {
            ...
            else // either active or locked-in... these states are visually very similar
            {
                ...
                m_PlayerNumberHolder.sprite = ClientCharacterSelectState.Instance.m_IdentifiersForEachPlayerNumber[m_PlayerNumber].Indicator;
                m_ActiveStateVisuals.SetActive(true);

                m_PlayerNameHolder.gameObject.SetActive(true);
                m_PlayerNameHolder.color = ClientCharacterSelectState.Instance.m_IdentifiersForEachPlayerNumber[m_PlayerNumber].Color;
                m_Button.interactable = m_IsDisabled ? false : true;

                if (m_State == NetworkCharacterSelection.SeatState.LockedIn)
                {
                    m_Glow.color = ClientCharacterSelectState.Instance.m_IdentifiersForEachPlayerNumber[m_PlayerNumber].Color;
        ...
        {
            ClientCharacterSelectState.Instance.OnPlayerClickedSeat(m_SeatIndex);
        }
